...
module.exports = {
  up: async (query) => {
    ...
    try {
      ...
      await query.sequelize.query("INSERT INTO user_role(user_id, role_id) SELECT u.id, r.id FROM user AS u, role AS r WHERE u.role = 'uploader' AND r.name IN ('uploader', 'viewer')", { type: query.sequelize.QueryTypes.INSERT, transaction, raw: true });
